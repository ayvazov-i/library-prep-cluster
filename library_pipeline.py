...
try:
    # dimorphite-dl 2.x: top-level import, ph_min/ph_max kwargs, returns list[str]
    from dimorphite_dl import protonate_smiles as _dimorphite_protonate
    def dimorphite_protonate(smi, min_ph=6.4, max_ph=8.4, precision=0.0):
        return _dimorphite_protonate(smi, ph_min=min_ph, ph_max=max_ph, precision=precision)
except ImportError:
    try:
        # dimorphite-dl 1.x fallback: submodule import, min_ph/max_ph kwargs
...
def apply_filters(df, pains_backend="auto", canon_tautomer=True):
    ...
    When canon_tautomer is True (default), each molecule is canonicalised with
    TautomerEnumerator.Canonicalize() before BRENK/PAINS matching. Set False to
    disable for A/B testing the canonicalisation step.
    ...
    for orig_idx, row in df.iterrows():
        ...
        # Disabled when canon_tautomer=False (used by --no-canon-tautomer A/B runs).
        if canon_tautomer:
...
def main():
    ...
    parser.add_argument("--no-canon-tautomer", action="store_true",
                        help="Disable tautomer canonicalisation before BRENK/PAINS "
                             "(for A/B testing the canonicalisation step).")
    ...
    df, failed_df = apply_filters(
        df,
        pains_backend=args.pains_backend,
        canon_tautomer=not args.no_canon_tautomer,
    )
    if args.save_intermediates:
